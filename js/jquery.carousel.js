;(function($){
   ...
   Carousel.prototype.setCircle=function(){
      if(this.options.circle.isshow){
        ...
        $carCircle.find('.circle').css({'width': this.options.circle.cird,'height': this.options.circle.cird,'background':'#fff','border-radius':'50%','cursor':'pointer'});
        if(this.type.indexOf('slidex')!=-1 || this.type=='opacx'){//圆点在底边
          ...
          $carCircle.find('.circle').css({'margin':'7px 7px'});
          $carCircle.css({
            'top':(this.carheight-this.carNum*(this.options.circle.cird+6))/2,
            'left':this.carwidth-(14+this.options.circle.cird)
          });
        }
        this.$element.find('.circle').eq(0).css({'background':'#fff','width': this.options.circle.cird, 'height':'50', 'border-radius':'10px'});
      }
   };
   //自动轮播器
   ...
   Carousel.prototype.setCarousel=function(current,prev,x){//current表示当前所指的小圆圈DOM对象，prev指的是当前元素对象的前一个索引值
      this.$element.find('.circle').css({'width': this.options.circle.cird,'height': this.options.circle.cird,'background':'#fff','border-radius':'50%','cursor':'pointer'});
      this.$element.find('.circle').eq(current).css({'background':'#fff','width': this.options.circle.cird, 'height':'50', 'border-radius':'10px'});
